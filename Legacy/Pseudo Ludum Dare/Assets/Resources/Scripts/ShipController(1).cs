	...
	void Update () {
		...
		if (gameControllerObject.GetComponent<GameController> ().gameState == 2) {
			...
			for (int i = 0; i < planetsList.Count; i++) {
				...
				float force;
				force = (float)0.0006 * shipMass * planetsList [i].GetComponent<Planet> ().gravFieldSize / (Mathf.Pow (ship.transform.position.x - planetsList [i].transform.position.x, 2) + Mathf.Pow (ship.transform.position.z - planetsList [i].transform.position.z, 2));
				...
				if (ship.transform.position.x - planetsList [i].transform.position.x < planetsList [i].GetComponent<Planet> ().size && planetsList [i].transform.position.x - ship.transform.position.x < planetsList [i].GetComponent<Planet> ().size &&
					ship.transform.position.z - planetsList [i].transform.position.z < planetsList [i].GetComponent<Planet> ().size && planetsList [i].transform.position.z - ship.transform.position.z < planetsList [i].GetComponent<Planet> ().size) {

					//TODO: Proper loss state

					gameControllerObject.GetComponent<GameController>().gameState = 4;

					Debug.Log ("M'friend you lost!");
				}
			}
			...
			if (shipVelocity != Vector3.zero)
				shipMesh.transform.rotation = Quaternion.LookRotation (-shipVelocity * Time.deltaTime * 10);
			else
